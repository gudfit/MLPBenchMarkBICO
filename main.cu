...
// Create an evaluator function for the BICO framework
Evaluator createEvaluator(const half *d_A, const half *d_W_up, const half *d_W_gate,
  ...
  return [=](const BudgetConfig& config) -> KernelResult {
    ...
    // Select kernel parameters based on budget configuration
    int block_k = 64;  // Default value
    int num_warps = 4; // Default value
    ...
    // Calculate required shared memory - FIXED calculation
    size_t smem_size = (WMMA_M * block_k + 2 * block_k * WMMA_N) * sizeof(half) +
                      2 * TILE_ELEMENTS * sizeof(float) + 
                      TILE_ELEMENTS * sizeof(half);
    
    // Check if configuration exceeds device limits
    int dev_id;
    cudaDeviceProp prop;
    ...
    if (smem_size > prop.sharedMemPerBlock) {
      ...
      return KernelResult(config, 0, false); // Invalid configuration
    }
    
    dim3 grid_fused((N + WMMA_N - 1) / WMMA_N, (M + WMMA_M - 1) / WMMA_M);
    dim3 block_size(32 * num_warps); // 32 threads per warp
    ...
    try {
      ...
      if (block_k == 32 && num_warps == 2) {
        glu_fused_up_gate_swiglu_budget<32, 2><<<grid_fused, block_size, smem_size>>>(
          d_A, d_W_up, d_W_gate, d_gated_result, M, N, K, 
          config.smem_budget, config.occ_budget);
      } else if (block_k == 128 && num_warps == 8) {
        glu_fused_up_gate_swiglu_budget<128, 8><<<grid_fused, block_size, smem_size>>>(
          d_A, d_W_up, d_W_gate, d_gated_result, M, N, K, 
          config.smem_budget, config.occ_budget);
      } else {
        glu_fused_up_gate_swiglu_budget<64, 4><<<grid_fused, block_size, smem_size>>>(
          d_A, d_W_up, d_W_gate, d_gated_result, M, N, K, 
          config.smem_budget, config.occ_budget);
      }
      ...
      // Actual measurement with multiple runs for averaging
      CUDA_CHECK(cudaEventRecord(start));
      for (int i = 0; i < 5; ++i) {  // Reduced from 10 to 5 for stability
        if (block_k == 32 && num_warps == 2) {
          glu_fused_up_gate_swiglu_budget<32, 2><<<grid_fused, block_size, smem_size>>>(
            d_A, d_W_up, d_W_gate, d_gated_result, M, N, K, 
            config.smem_budget, config.occ_budget);
      ...
      latency_ms /= 5.0f; // Average latency
      
      // Run down projection kernel
      dim3 grid_down((K + WMMA_K - 1) / WMMA_K, (M + WMMA_M - 1) / WMMA_M);
      glu_kernel3_down_gemm<<<grid_down, dim3(32)>>>(
        d_gated_result, d_W_down, d_output, M, N, K);
...
int main() {
  ...
  // BENCHMARK 1: Baseline 3-Kernel Implementation
  ...
  // Create BICO explorer with evaluator
  auto evaluator = createEvaluator(d_A, d_W_up, d_W_gate, d_gated_result, 
                                 M, N_inter, K_hidden, d_W_down, d_output);
  BICOExplorer explorer(evaluator);
  
  // Explore budget frontier
  ...
  if (final_result.valid) {
    ...
    if (final_result.latency_ms > 0)
      std::cout << "Speedup from Optimization: " << (ms_baseline / final_result.latency_ms) << "x"
                << std::endl;
  } else {
