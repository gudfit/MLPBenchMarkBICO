...
void BICOExplorer::exploreFrontier(int max_iterations) {
    ...
    std::vector<BudgetConfig> initial_configs = {
        BudgetConfig(32, 0.25f),   
        BudgetConfig(64, 0.5f),    
        BudgetConfig(96, 0.75f),   
        BudgetConfig(48, 0.9f),    
        BudgetConfig(128, 0.3f)    
    };
    ...
    std::cout << "Pareto frontier found with " << frontier.size() << " configurations" << std::endl;
}

BudgetConfig BICOExplorer::findBestConfig() const {
    if (results.empty()) return BudgetConfig();
